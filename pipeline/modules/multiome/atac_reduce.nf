process MULTIOME_ATAC_REDUCE {
    ...
    val predecessor
    output:
    val "multiome_02_reduce_atac.h5ad", emit: checkpoint
    script:
    """
    /opt/conda/envs/omicsage/bin/python - << 'PYEOF2'
import yaml, pathlib, sys
cfg  = yaml.safe_load(open('/app/${config_path}'))
keys = "steps.atac_reduce".split('.')
node = cfg
for k in keys:
    node = node.get(k, {}) if isinstance(node, dict) else {}
enabled = node.get('enabled', True) if isinstance(node, dict) else True
if not enabled:
    print('[atac_reduce] disabled in config -- skipping')
    pathlib.Path('multiome_02_reduce_atac.h5ad').touch()
    sys.exit(0)
PYEOF2
