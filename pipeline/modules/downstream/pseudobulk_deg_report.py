"""
pseudobulk_deg_report.py — HTML report generator for OmicSage pseudobulk DEG module
Phase 1, Step 10

Generates a self-contained HTML report from pseudobulk_deg() output.
Matches the style of deg_report.py exactly (same CSS, same page skeleton).
Adds two pseudobulk-specific sections not present in deg_report.py:
  - Pseudo-sample counts table: (cell_type, donor) → n_cells, included/excluded
  - Skipped groups table: cell types that had too few donors and why
...
def generate_pseudobulk_deg_report(
    adata: AnnData,
    pb_dict: dict,
    output_path: str = "reports/pseudobulk_deg_report.html",
    top_n_volcano: int = 10,
    top_n_dotplot: int = 5,
    max_volcano_groups: int = 20,
) -> str:
    """
    Generate a self-contained HTML report for pseudobulk DEG results.

    Parameters
    ----------
    adata : AnnData
        AnnData returned by pseudobulk_deg(), containing
        adata.uns['omicsage_pseudobulk_deg'].
    pb_dict : dict
        pb_dict returned by pseudobulk_deg().
        Keys: results, summary_df, provenance, pairwise, skipped.
    output_path : str
    ...
    max_volcano_groups : int
        ...
        Groups with most DEGs are shown first when limit is exceeded.
    ...
    provenance = pb_dict["provenance"]
    results    = pb_dict["results"]
    summary_df = pb_dict["summary_df"]
    skipped    = pb_dict.get("skipped", {})

    sections = []
    sections.append(_section_summary_stats(provenance, results, skipped))
    sections.append(_section_skipped_groups(skipped))
    ...
    sections.append(_section_dotplot(adata, results, provenance, top_n=top_n_dotplot))

    html = _render_page(
        title="OmicSage — Pseudobulk DEG Report",
        sections=sections,
...
def _section_summary_stats(
    ...
    method        = provenance.get("method", "DESeq2 Wald test (pydeseq2)")
    groupby       = provenance.get("groupby", "—")
    donor_key     = provenance.get("donor_key", "—")
    counts_layer  = provenance.get("counts_layer", "—")
    min_cells     = provenance.get("min_cells", "—")
    min_samples   = provenance.get("min_samples", "—")
    min_logfc     = provenance.get("min_logfc", "—")
    max_pval_adj  = provenance.get("max_pval_adj", "—")
    n_groups      = provenance.get("n_groups", len(results))
    n_skipped     = provenance.get("n_skipped", len(skipped))
    timestamp     = provenance.get("timestamp", "—")
    exclude_pfx   = provenance.get("exclude_gene_prefixes", [])
    ...
    stat_cards = "".join(
        ...
        for k, v in [
            ...
            ("Min cells / pseudo-sample", min_cells),
            ("Min donors / cell type",  min_samples),
    ...
    per_group_rows = "".join(
        ...
        for group, count in sorted(
            {g: len(df) for g, df in results.items()}.items()
        )
    )

    exclude_note = ""
    if exclude_pfx:
        prefix_str = ", ".join(exclude_pfx)
        exclude_note = (
    ...
    pb_note = (
        '<p class="note">'
        "ℹ Pseudobulk DEG aggregates raw counts per (cell type, donor) into "
        "bulk-like samples and runs DESeq2 Wald tests. This controls for "
        "donor-level variation and is the recommended method for publication figures. "
        "See <strong>Step 6</strong> for the complementary Wilcoxon results."
        "</p>"
    )
...
def _section_skipped_groups(skipped: dict) -> str:
    """Report on cell types that were skipped and why."""
    if not skipped:
        return (
            '<section><h2>Skipped Groups</h2>'
            '<p>All cell types had sufficient donor pseudo-samples — none skipped.</p>'
            '</section>'
        )

    rows = "".join(
        f"<tr><td>{group}</td><td>{reason}</td></tr>"
        for group, reason in sorted(skipped.items())
    )
    return f"""
    <section>
      <h2>Skipped Groups</h2>
      <p>
        Cell types below were skipped because they had too few donor pseudo-samples
        after the <code>min_cells</code> filter.  DESeq2 requires replicates —
        increase <code>min_samples</code> threshold or collect more donors to include
        these groups.
      </p>
      <table>
        <thead><tr><th>Cell Type</th><th>Reason</th></tr></thead>
        <tbody>{rows}</tbody>
      </table>
    </section>
    """
...
def _section_summary_table(summary_df: pd.DataFrame) -> str:
    if summary_df.empty:
        return (
            "<section><h2>Top DEGs per Group</h2>"
            "<p>No significant DEGs found.</p></section>"
        )
    ...
    for _, row in summary_df.iterrows():
        ...
        if row["group"] != current_group:
            ...
            group_cell = (
                f'<td rowspan="{count}" class="group-cell">{row["group"]}</td>'
            )
    ...
    <section>
      ...
      <p>Top 5 significant genes per group, ranked by adjusted p-value (DESeq2 padj).</p>
      <table>
        <thead>
          <tr>
...
def _section_global_volcano(
    ...
    """Single volcano plot pooling DESeq2 results from all groups combined."""
    ...
    <section>
      ...
      <p>
        ...
        (DESeq2 results pooled). Dashed lines mark thresholds: |log₂FC| ≥ {min_logfc},
        adj. p ≤ {max_pval_adj}. Top {top_n} genes by significance are labelled.
      </p>
...
def _section_volcano_plots(
    ...
    <section>
      <h2>Volcano Plots</h2>
      <p>
        Each plot shows log₂ fold-change vs. −log₁₀(adjusted p-value) from DESeq2.
...
def _section_dotplot(
    ...
    """
    Dot plot of top N pseudobulk DEGs per group using sc.pl.dotplot.
    Reads expression from adata.layers['logcounts'] for display.
    Falls back gracefully if adata or logcounts are unavailable.
    """
    groupby = provenance.get("groupby", None)
    ...
    # Use logcounts layer for display if present; fall back to X
    plot_adata = adata.copy()
    if "logcounts" in plot_adata.layers:
        plot_adata.X = plot_adata.layers["logcounts"]

    try:
        fig, ax = plt.subplots(
            figsize=(max(6, len(valid_genes) * 0.5 + 2), 4)
        )
        sc.pl.dotplot(
            plot_adata,
            ...
            ax=ax,
            show=False,
            title=f"Top {top_n} pseudobulk DEGs per group",
        )
        img_b64  = _fig_to_base64(fig)
        img_html = (
            f'<img src="data:image/png;base64,{img_b64}" '
            'alt="Dot plot top pseudobulk DEGs" style="max-width:100%;">'
        )
        plt.close(fig)
    ...
    return f"""
    <section>
      <h2>Dot Plot — Top {top_n} Pseudobulk DEGs per Group</h2>
      <p>
        Dot size encodes fraction of cells expressing the gene.
        Colour encodes mean expression in expressing cells (from
        <code>layers['logcounts']</code>).
        Genes are the top {top_n} from DESeq2 results per group.
      </p>
      {img_html}
    </section>
    """
...
# ---------------------------------------------------------------------------
# HTML renderer  (identical CSS to deg_report.py)
# ---------------------------------------------------------------------------
...
<body>
  <header>
    <h1>OmicSage — Pseudobulk DEG Report</h1>
    <p>Generated {timestamp} · DESeq2 via pydeseq2</p>
  </header>
  <main>
    {body}
  </main>
